...
export class ContaRepository {
    ...
    async insereConta(conta: Conta): Promise<Conta> {
        const query = "INSERT INTO banco.conta (numeroConta, saldo, tipoConta) VALUES (?, ?, ?)";
        try {
            const resultado = await executarComandoSQL(query, [conta.numeroConta, conta.saldo, conta.tipoConta]);
    ...
    updateConta(conta: Conta) {
        const query = 'UPDATE banco.Conta SET numeroConta = ?, saldo = ?, tipoConta = ? WHERE ID = ?';
        executarComandoSQL(query, [conta.numeroConta, conta.saldo, conta.tipoConta, conta.id])
    ...
    async atualizaTipoConta(tipoConta: number, novoTipoConta: number) {
        try {
            ...
            const contasAfetadas: number = (await this.getContaPorIdOuNumeroOuTipo(undefined,undefined,tipoConta)).length;
            if (contasAfetadas > 0) {
    ...
    async getContaPorIdOuNumeroOuTipo(id?: number, numeroConta?: number, tipo?: number): Promise<Conta[]> {
