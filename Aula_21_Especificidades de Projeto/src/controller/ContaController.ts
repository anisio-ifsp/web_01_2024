...
export async function getConta(req: Request, res: Response) {
    try {
        const conta = await serviceConta.getConta(req.query.id, req.query.numeroConta, req.query.tipoConta, req.query.cliente);
        res.status(200).json(
            {
                conta: conta
            }
...
export async function getContas(res: Response) {
    try {
