...
export class TipoContaService{
    ...
    async criaTipoConta(descricaoData: any):Promise<TipoConta>{
        let {descricao} = descricaoData;
        if (!descricao) {
            throw new Error("A descrição deve ser informada.");
        }

        descricao = String(descricao).toLocaleLowerCase();
    ...
    async deletaTipoConta(tipoConta: TipoConta):Promise<TipoConta>{
        ...
        const listaContas: Conta[] = await this.contaRepository.getContaPorIdOuNumeroOuTipoOuCliente(undefined, undefined, result[0].codigoTipoConta)
